...
import { resolveDataAtPath } from "@app/utils/resolveDataAtPath";
import { createSearchScoper } from "@app/context/data/scopers/create-search-scoper";
import { applyDataScopers } from "@app/context/data/apply-data-scopers";
...
const ResolvedDataContext = createContext< ResolvedDataState | undefined >( undefined );

export const ResolvedDataProvider = ( { children }: PropsWithChildren ) => {
    const { data: rootData } = useProvider();
    const hasNoData = ! Boolean( rootData )

    const data = applyDataScopers( rootData );
    ...
    return (
        <ResolvedDataContext.Provider value={ { data, rootData, hasNoData } }>
            { children }
        </ResolvedDataContext.Provider>
    );
};

export const useResolvedData = () => {
    const context = useContext( ResolvedDataContext );

    if ( ! context ) {
        throw new Error( "useResolvedData must be used within a ResolvedDataProvider" );
    }
...
//       createSelectedKeyFilterScope(selectedKey),
//       createSearchScoper(path),
//     ];
//
//     return scopers.reduce(
//       (current, scope) => scope(current),
//       rootData
//     );
//   }, [rootData, path, selectedKey]);


//TODO: remove hooks form anoymous fxn
...
// console.log( data );
// const getValueAtPath = createPathValueGetter( rootData );
...
//     }
//
//     const exactMatch = getValueAtPath( path );
//
//     if ( exactMatch !== undefined ) {
//         return exactMatch;
//     }
//
//     return resolveDataAtPath( path, getValueAtPath ) ?? rootData;
// }, [ rootData, path, getValueAtPath ] );
...
// const getPathValue = createPathValueGetter( data );
// TODO: suggestions ar not supposed to be part of here
// const suggestions = useSuggestions( paths, getPathValue );
